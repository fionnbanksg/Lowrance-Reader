...
def sl3_decode(data):
    ...
    while (position < len(data)):
        ...
        frame_size_prev = int.from_bytes(head[10:12], "little", signed = False)
        sounding_size = int.from_bytes(head[44:46], "little", signed = False)
        time = int.from_bytes(head[124:128], "little", signed = False)
        ...
        if frame_size != 0:
            ...
            position+=frame_size_prev
        if sounding_size == 3072:
            print('Current Frame Size: ', frame_size)
            print('Time:', time)
            print('Time-dif: ', time-prev_time)
            prev_time = time
