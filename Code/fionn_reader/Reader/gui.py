...
def open_sl_file():
    ...
    if file_path:
        ...
        dataframe = df 
        dataframe = df.query("survey_label == 'primary'") 
        print('completed')
        primary_list = [np.frombuffer(sl_bin_data[(f+168):(f+(p-168))], dtype="uint8") for f, p in zip(dataframe["first_byte"], dataframe["frame_size"])]
        print('completed')
        #for i, arr in enumerate(primary_list):
            #print(f"Array {i + 1} length:", arr.shape[0])
        filtered_list = [arr for arr in primary_list if arr.shape[0] == 2904]

        # Stack the filtered arrays
        if filtered_list:
            primary_np = np.stack(filtered_list)
            print("Stacked array shape:", primary_np.shape)
        else:
            print("No arrays with width 2904 found.")
...
export_menu.add_command(label="Export Other Data", command=lambda:export_other_data(primary_np), state="disabled")
export_menu.add_command(label="Process Data", command=lambda:process_data(primary_np), state="disabled")
...
intensity_var = tk.DoubleVar(value=5)  # Initial sensitivity threshold set to mid-gray
...
intensity_scale = ttk.Scale(root, from_=0, to=10, variable=intensity_var, orient="horizontal")
